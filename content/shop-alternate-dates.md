---
title: Поиск перелетов по гибким датам
aliases:
    - /alternate-date
---
...
Возможные значения:
- ```AD1``` — поиск рекомендаций с датой вылета (или вылетов для маршрутов туда-обратно) ± 1 день (матрица лучших цен 3x3)
- ```AD3``` — поиск рекомендаций с датой вылета (или вылетов для маршрутов туда-обратно) ± 3 дня (матрица лучших цен 7x7)
- ```AD7``` — поиск рекомендаций с датой вылета (или вылетов для маршрутов туда-обратно) ± 7 дней или рекомендаций с гибкой датой вылета (матрица лучших цен c изменяемыми размерами)
...
где каждая буква обозначает самую дешевую рекомендацию для заданной пары дат.
...
Для запросов типа ```AD1``` и ```AD3``` ответ будет содержать 9 или 49 рекомендаций, соответственно, для маршрута туда-обратно (Round Trip), и 3 и 7 рекомендаций, соответственно, для маршрута в одну сторону (One Way), т.е. по одной самой дешевой рекомендации для каждой даты.

Для запроса типа ```AD7``` по умолчанию ответ будет содержать 225 (Round Trip) или 15 (One Way) рекомендаций. Однако, в атрибутах элемента ```/OTA_AirLowFareSearchRQ/TravelPreferences/TPA_Extensions/NumTrips``` можно дополнительно указать требование к количеству возвращаемых рекомендаций:
- ```/@Number``` — общее количество рекомендаций в ответе
- ```/@PerDateMax``` — максимальное количество рекомендаций для каждой пары дат вылета (Round Trip) или даты вылета (One Way)
- ```/@PerDateMin``` — минимальное количество рекомендаций для каждой пары дат вылета (Round Trip) или даты вылета (One Way)

## Примеры

{{< details title="Пример запроса (± 3 дня)" >}}
```XML
...
{{< details title="Пример запроса (± 7 дней)" >}}
```XML
<OTA_AirLowFareSearchRQ ResponseType="OTA" Version="6.1.0" xmlns="http://www.opentravel.org/OTA/2003/05">
  <POS>
    <Source PseudoCityCode="9LSC">
